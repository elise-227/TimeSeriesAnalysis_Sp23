...
# short hand to get the reamining columns 2 + number of columns - 1 
ts_inflow_data
...
```{r}
#run autocorrelation on HP1 
acf(ts_inflow_data[,1], lag.max =50)
#each bar represents the autocorrelation
#0 is always 1 -> Yt autocorrelation to Yt

#seasonal component 
#draws up to 30 legs, sig correlations up to 30 legs 
#blue lines are the significant values, within blue is not significant 
...
```{r}
inflow_sub <- inflow_data[(year(inflow_data$my_date) < 2011),]
```
