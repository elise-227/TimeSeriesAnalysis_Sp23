...
ts <- ts(inflow_data[,2:16], start = c(year(first(inflow_data$my_date)),month(first(inflow_data$my_date))), frequency = 12)
head(ts)
```
...
for(i in 1:nhydro){
  par(mfrow=c(1,2))
  acf(ts[,i], lag.max = 40, main = paste0("HP", i))
  pacf(ts[,i], lag.max = 40, main = paste0("HP", i))
}

# acf(ts[,1], lag.max = 40)
# pacf(ts[,1], lag.max = 40)
```
...
# dummy <- seasonaldummy(ts[,4])

#Fit a linear trend to TS of iHP
lm <- lm(inflow_data[,5]~t)
summary(lm)
