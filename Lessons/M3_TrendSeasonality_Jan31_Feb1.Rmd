...
detrend <- (inflow_data[,iHP+1]) - (beta0+beta1*t)
  ...
  #(ts_inflow_data - linear_trend$coefficients)
ggplot(inflow_data, aes(x = my_date, y = inflow_data[,iHP+1]))+
  geom_line(color="blue")+
  geom_line(aes(y=detrend), color = "green")

#easier way to detrend
plot(linear_trend$residuals, type="l", col = "green")
...
dummy <- seasonaldummy(ts_inflow_data[,iHP+1])
smm <- lm(inflow_data[,iHP+1] ~ dummy)
summary(smm)
#all relative to December 
#neg because relative to December 
beta0 <- smm$coefficients[1]
beta1 <- smm$coefficients[2:12]

 inflow_seas_com <- array(0,nobs)
for (i in 1:nobs){
  inflow_seas_com[i] = beta0 + beta1 %*% dummy[i,]
}
   

ggplot(inflow_data, aes(x = my_date, y = inflow_data[,iHP+1]))+
  geom_line(color="blue")+
  geom_line(aes(y=inflow_seas_com), color = "red")
  #geom_line(aes(y=detrend), color = "green")

#check back on notes for how to do 
```
...
SMtest <- SeasonalMannKendall(ts_inflow_data[,iHP])
SMtest

#null stationary and no trend

#our data has a trend sig p value 

#Use yearly date to run Mann Kendall

#explosive = phi greater than one , we wil always check for stationarity 
