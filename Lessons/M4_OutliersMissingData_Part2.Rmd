...
d_ts_wind_a <- decompose(clean_wind_data)
d_ts_wind_m <- decompose(clean_wind_data, "multiplicative")
plot(d_ts_wind_a)
plot(d_ts_wind_m)
#Ex2: Store the three component in separate vector

s <- d_ts_wind_a$seasonal
t <- d_ts_wind_a$trend
r <- d_ts_wind_a$random

#Ex3: Create one data frame with all three vector and the Date columns from wind_data

all <- as.data.frame(cbind(s,t,r))
all <- cbind(all, full_wind_data$Date)

#Ex4: Inspect the data frame using head() and tail()
head(all)
tail(all)

#center moving average so missing six observations, loose six on the trend 
#stl the smaller the bar the most representative 
